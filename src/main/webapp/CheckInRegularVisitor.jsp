...
<head>
<meta charset="UTF-8">
<title>Add Regular Visitor</title>
<style>
body {
...
<div class="login-form">
            ...
            <form action="" method="post">
                    ...
                    <input type="tel" name="SecurityCode" pattern="^[0-9]{6}$" title =" t should be 10 digits"  required>
                    <br><br>
...
<% 
		...
		try(Connection con=DriverManager.getConnection(url,user,paswd)){
			int Code=Integer.parseInt(request.getParameter("SecurityCode"));
			PreparedStatement prp1=con.prepareStatement("Select * from RegularVisitor_Details where SecurityCode=?");
			prp1.setInt(1,Code);
			ResultSet rs=prp1.executeQuery();
			if(rs.next()){
			int FlatNo=Integer.parseInt(request.getParameter("FlatNo"));
			PreparedStatement prp =con.prepareStatement("Insert into RegularVisitorTimings(SecurityCode,FlatNo,BuildingName) values (?,?,?)") ;	
			prp.setInt(1,Code);
			prp.setInt(2,FlatNo);
			prp.setString(3,BuildingName);
			prp.executeUpdate();
			String a="<script>alert('Succesfully CheckedIn.')</script>";
			out.println(a);
				
			}else{
				out.println("<script>alert('Security Code doesnot exist')</script>");
			}
